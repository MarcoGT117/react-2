...
export const reducer = (state = initialState, action) => {
  switch (action.type) {
    case 'todo/add':
      ...
      return {
        ...state, 
         todos: state.todos.concat({id, ...action.payload}) // Se tiene que regresar una copia del state o si no la aplicacion no se renderizara
        }
    case 'todo/complete':
      const newTodos = state.todos.map(todo =>{
        if(todo.id === action.payload.id){
          return { ...todo, completed: !todo.completed} // Regresa una copia del todo pero modifica al contrario la propiedad completed
        }
        return todo;
      })
      return {
